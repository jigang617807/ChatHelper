...
public interface DocumentChunkRepository extends JpaRepository<DocumentChunk, Long> {
    ...
    @Query(value = "SELECT id, text, chunk_index AS \"chunkIndex\", page_number AS \"pageNumber\", section_title AS \"sectionTitle\", content_type AS \"contentType\", source_path AS \"sourcePath\", (embedding <=> cast(cast(:queryVector as text) as vector)) AS distance FROM document_chunk WHERE document_id = :docId ORDER BY embedding <=> cast(cast(:queryVector as text) as vector) LIMIT :limit", nativeQuery = true)
